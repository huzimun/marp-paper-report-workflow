...
- Modify only pages named by the user or pages with explicit placeholders such as `鍥捐〃鍗犱綅`.
...
- Start each figure explanation with the paper and exact figure/table number, for example `Steyvers et al. Fig. 1:`. Do not use generic openings such as `璇诲浘锛歚.
- Ensure each figure explanation satisfies four checks: it matches the slide `鐢ㄩ€擿 and `鎴浘寤鸿`; it reflects the original paper figure/table title or caption; it fits the surrounding slide text; and it is independently understandable from the image plus explanation alone.
...
- If the screenshot suggestion requests Chinese labels or callouts, create an annotated version of the figure. Prefer GPT image editing when the environment supports passing the original image as input; otherwise use a reproducible local method such as SVG/HTML/Canvas overlays, and save the annotated asset under `figures/`. Always inspect the exported PDF/PNG; if SVG text renders incorrectly or fonts are missing, rasterize the annotated figure to PNG.
